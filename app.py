...
def mark_bulk_attendance():
    ...
    if not subject_id or not semester or not class_date or not records:
    ...
    subj = subjects_col.find_one({"_id": oid})
    if not subj:
    ...
    for rec in records:
        ...
        attendance_col.update_one(
            {"name": name, "class_date": class_date, "subject": subject_name, "semester": semester},
            {"$set": {
                "name": name,
...
def get_my_attendance(name):
    docs = attendance_col.find(
        {"name": name},
        {"_id": 0, "name": 1, "class_date": 1, "subject": 1, "semester": 1, "status": 1}
    )
    data = []
    for d in docs:
        data.append({
...
def save_marks():
    ...
    if not subject_id:
        return jsonify({"message": "Subject ID missing ❌"}), 400
    ...
    if subj.get("teacher_name") != teacher_name:
        return jsonify({"message": "Unauthorized: You are not assigned to this subject ❌"}), 403
